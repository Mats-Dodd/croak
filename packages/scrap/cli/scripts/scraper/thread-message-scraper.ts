...
export async function threadMessageScraper() {
  ...
  async function fetchAndSaveMessages(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `HTTP error! status: ${response.status} channelId: ${channelId}`
        );
      }

      const rawData = await response.json();
      const data = MessageResponseSchema.parse({
        messages: rawData,
        has_more: rawData.length === 100,
      });
  ...
  async function processChannel(channelId: string) {
    ...
    while (hasMore) {
      const { count, hasMore: moreMessages } = await fetchAndSaveMessages(
        channelId,
        lastMessageId
      );
      totalMessages += count;
      ...
      console.log(
        `Fetched and saved ${count} messages for channel ${channelId}. Total: ${totalMessages}`
      );
    }

    // Mark the channel as complete
    await db
      .update(discordChannel)
      .set({ thread_data_complete: true })
      .where(eq(discordChannel.id, channelId));

    console.log(
      `Channel ${channelId} processing complete. Total messages: ${totalMessages}`
    );
  }

  try {
    while (true) {
      const incompleteChannels = await db
        .select({ id: discordChannel.id })
        .from(discordChannel)
      ...
      if (incompleteChannels.length === 0) {
        console.log(
          "No more incomplete channels. Waiting 5 seconds before trying again..."
        );
